...
$type._resolveUrl = function WebRequest$_resolveUrl(url, baseUrl) {
    ...
    baseUrl = baseUrl.substr(0, baseUrl.lastIndexOf('http://azredistricting.org/') + 1);

    if (!url || !url.length) {
        return baseUrl;
    }

    if (url.charAt(0) === 'http://azredistricting.org/') {
        var slashslash = baseUrl.indexOf('://');
        if (slashslash === -1) {
            throw Error.argument("baseUrl", Sys.Res.badBaseUrl1);
        }

        var nextSlash = baseUrl.indexOf('http://azredistricting.org/', slashslash + 3);
        if (nextSlash === -1) {
        ...
        var lastSlash = baseUrl.lastIndexOf('http://azredistricting.org/');
        if (lastSlash === -1) {
